...
function Account_category() {
  ...
  const onSubmitHandler = (e) => {
    ...
    var mdl_account_category = {
      id: id, name: name
    }
    if (id) {
      Commons.updateUnit(mdl_account_category, id).then((res) => {
        resetAfterSave()
      })
    } else {
      Commons.saveAccount_category(mdl_account_category).then((res) => {
  ...
  return (
    <PagesWapper>
      ...
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerRowBtwn clearBtn={clearBtn} form='Role' showLoader={showLoader}  >
          <ClearBtnSaveStatus height={height} showLoader={showLoader} showAlert={showAlert} />
          <FormInnerRightPane onSubmitHandler={onSubmitHandler}>

            <InputRow name='Role Name' val={name} handle={(e) => setName(e.target.value)} label='lblName' />
      ...
      <ContainerRow mt='3'>
        <ListToolBar listTitle='Roles List' height={height} entity='Unit' changeFormHeightClick={() => setHeight(height === 0 ? 'auto' : 0)} changeSearchheight={() => setSearchHeight(searchHeight === 0 ? 'auto' : 0)} handlePrint={handlePrint} searchHeight={searchHeight} />
