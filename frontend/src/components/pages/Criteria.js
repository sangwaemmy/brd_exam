...
function Criteria() {
  ...
  const [criteria_group, setCriteria_group] = useState()
  ...
  const onSubmitHandler = (e) => {
    ...
    var mdl_criteria = {
      id: id, name: name, criteria_group: criteria_group
    }
    if (id) {
  ...
  return (
    <PagesWapper>
      ...
      <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
        <ContainerRowBtwn clearBtn={clearBtn} form='Criteria' showLoader={showLoader}  >
          <ClearBtnSaveStatus height={height} showLoader={showLoader} showAlert={showAlert} />
          <FormInnerRightPane onSubmitHandler={onSubmitHandler}>

            <InputRow name='Name ' val={name} handle={(e) => setName(e.target.value)} label='lblName ' />
            <InputRow name='Criteria Group ' val={criteria_group } handle={(e) => setCriteria_group (e.target.value)} label='lblCriteria Group ' />
      ...
      <ContainerRow mt='3'>
        <ListToolBar listTitle='Criteria List' height={height} entity='Unit' changeFormHeightClick={() => setHeight(height === 0 ? 'auto' : 0)} changeSearchheight={() => setSearchHeight(searchHeight === 0 ? 'auto' : 0)} handlePrint={handlePrint} searchHeight={searchHeight} />
